...
from xtr_lock import ConsensusStrategy
...
def test_met_on_success(success: int, failure: int, total: int, expected: bool) -> None:
    del failure
    assert ConsensusStrategy().is_met(success, total) is expected
...
def test_can_be_met(success: int, failure: int, total: int, expected: bool) -> None:
    del success
    assert ConsensusStrategy().can_be_met(failure, total) is expected
